...
`include "c5x72.v"
`include "/apps/synopsys/Y-2006.06-SP1/dw/sim_ver/DW02_mult_3_stage.v" 
module top();
...
always @(posedge(clk)) begin
    ...
    if(!reset) begin
        ...
        rd = res;
	ed=efifo.pop_front();
        $display("1.res %h rst%b p%bs%b mrfinal%h ed %h rd_data %h wr_data %h",rd,reset,pushout,stopout,c.mrfinal,ed,c.f.data_out,c.mrfinal);
